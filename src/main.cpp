...
uint256 hashGenesisBlock("0xc3fb332f2ba73683123a72f69de2c9d2db2e1437b44916afe8cf6f5e2dce30bc");
static CBigNum bnProofOfWorkLimit = CBigNum().SetCompact(503416870); // Bacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bacoin Signed Message:\n";
...
{
    // Bacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 500000000;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 55000); // Bacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 32400; // Bacoin: 3.5 days
static const int64 nTargetSpacing = 150; // Bacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Bacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc9735fa700b457648ef94faa3877e29db7f43974822beb889a1aeff35661cde5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "7/13/17 Bacon is good for me.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 500000000;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("f58739b162984341dff3e89b20c307675531fe53ed7df198729d1e7fab1e82b72e6ded74e8567f9b6f93ce4fb7daf4ca7c92170c304888be2ddf3e2d3ba51cc976") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1499963067;
        block.nBits    = 503416870;
        block.nNonce   = 419459990;

        if (fTestNet)
        {
            block.nTime    = 1499963067;
            block.nNonce   = 3977492724;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x5bf5d09d620110856e3d89ee79c1854fcfb62cc87359f507f15f0c37f1feca64"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BacoinMiner : ProcessBlock, block not accepted");
    }
